...
#ifndef GPIO_TEST
/* RT584 5x5 EVB layout has no GPIO 2 & GPIO 3 */
#define GPIO_TEST (1)
#endif
...
#if (DTM_UART == UART_0)
#define DTM_UART_TX_IO 17
#define DTM_UART_RX_IO 16
...
void dtm_sys_gpio_init(void) {
    ...
    pin_set_pullopt(GPIO_TEST, PULL_UP_100K);
    ...
    pin_set_pullopt(PWR_LEVEL0_KEY,
                    PULL_UP_100K); /*Configure PWR_LEVEL0_KEY 100K pull-up*/
    gpio_cfg_input(
        PWR_LEVEL0_KEY,
...
#if 0
    /* TBD: Equivalence of logc in RT584 ? */
    SYSCTRL->SYS_LOWPOWER_CTRL |= 0x80;
    SYSCTRL->SYS_LOWPOWER_CTRL &= (~(1 << 7)); /* Disable SRAM auto deep-sleep control. */
...
#if (SLEEP_TRIGGER_VALUE == 0)
        pin_set_pullopt(GPIO_TEST, PULL_DOWN_100K);
#endif
