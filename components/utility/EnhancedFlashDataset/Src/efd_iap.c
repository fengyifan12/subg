 ...
 * @return result
 */
EfErrCode efd_erase_spec_user_app(uint32_t user_app_addr, size_t app_size,
 ...
 * @return result
 */
EfErrCode efd_write_data_to_bak(uint8_t* data, size_t size, size_t* cur_size,
                                size_t total_size) {
    EfErrCode result = EFD_NO_ERR;
    ...
    result = efd_port_write(efd_get_bak_app_start_addr() + *cur_size,
                            (uint32_t*)data, size);
    switch (result) {
        case EFD_NO_ERR: {
            *cur_size += size;
 ...
 * @return size
 */
uint32_t efd_get_bak_app_start_addr(void) { return bak_app_start_addr; }
