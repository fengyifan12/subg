/*
 * app_task.c (miu-rgb) — RGB 灯带子设备主任务
 *
 * 基于 miu-mtd，添加 RGB PWM 控制（GPIO1/6/7 对应 R/G/B）。
 * 覆盖 app_udp_comm_json_process() 弱符号：
 *   收到 ACK     → 打印日志
 *   收到 CONTROL → 解析 data.r / data.g / data.b，调用 app_rgb_set()
 */
...
#define BIN_TYPE_ARR 'm', 'i', 'u', '-', 'r', 'g', 'b'
 ...
 *   ACK     → 打印确认日志
 *   CONTROL → 解析 data.r / data.g / data.b，调用 app_rgb_set()
 * ----------------------------------------------------------------------- */
                                ...
                                otIp6Address src_addr)
{
    ...
    if (miu_json_get_str(json, "type",     msg_type, sizeof(msg_type)) < 0 ||
        ...
        log_info("[rgb] rx: malformed JSON");
        return;
    }
    ...
    if (strcmp(msg_type, "ACK") == 0) {
        ...
        log_info("[rgb] << ACK  dev=%s  seq_ack=%d  code=%d",
                 dev_name, seq_ack, code);
        if (code == 0) {
            log_info("[rgb] REGISTER confirmed by leader!");
        }
    } else if (strcmp(msg_type, "CONTROL") == 0) {
        int r = -1, g = -1, b = -1;
        miu_json_get_int(json, "r", &r);
        miu_json_get_int(json, "g", &g);
        miu_json_get_int(json, "b", &b);

        if (r < 0 || g < 0 || b < 0) {
            log_info("[rgb] << CONTROL: missing r/g/b field(s): %s", json);
        } else {
            log_info("[rgb] << CONTROL R=%d G=%d B=%d", r, g, b);
            app_rgb_set((uint8_t)r, (uint8_t)g, (uint8_t)b);
            app_rgb_net_report();
        }
    } else {
        log_info("[rgb] << unknown type '%s'", msg_type);
    }
}
...
void app_common_init()
{
    ...
    log_info("Mesh It Up RGB device");
