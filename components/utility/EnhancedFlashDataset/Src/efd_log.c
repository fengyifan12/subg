...
static SectorStatus get_sector_status(uint32_t addr) {
    ...
    if (efd_port_read(header_addr, header_buf, sizeof(header_buf))
        == EFD_NO_ERR) {
...
static EfErrCode write_sector_status(uint32_t addr, SectorStatus status) {
    ...
    switch (status) {
        ...
        case SECTOR_STATUS_USING: {
            ...
            return efd_port_write(header_addr + sizeof(header), &header,
                                  sizeof(header));
        }
        case SECTOR_STATUS_FULL: {
            header = SECTOR_STATUS_MAGIC_FULL;
            return efd_port_write(header_addr + sizeof(header) * 2, &header,
                                  sizeof(header));
        }
        default: return EFD_WRITE_ERR;
    }
}
...
static void find_start_and_end_addr(void) {
    ...
    for (cur_size = EFD_ERASE_MIN_SIZE; cur_size < LOG_AREA_SIZE;
         cur_size += EFD_ERASE_MIN_SIZE) {
        ...
        switch (last_sec_status) {
            case SECTOR_STATUS_EMPUT: {
                switch (cur_sec_status) {
                    ...
                    case SECTOR_STATUS_USING:
                        EFD_DEBUG("Error: Log area error! Now will clean all "
                                  "log area.\n");
                        efd_log_clean();
                        return;
                    case SECTOR_STATUS_FULL:
                ...
                switch (cur_sec_status) {
                    case SECTOR_STATUS_EMPUT:
                        ...
                        if (cur_log_sec_state == 2) {
                            EFD_DEBUG("Error: Log area error! Now will clean "
                                      "all log area.\n");
                            efd_log_clean();
                            return;
                        } else {
                    ...
                    case SECTOR_STATUS_USING:
                        if (total_sec_num <= 2) {
                            ...
                            if (cur_size + EFD_ERASE_MIN_SIZE
                                >= LOG_AREA_SIZE) {
                                cur_log_sec_state = 2;
                        ...
                        break;
                    case SECTOR_STATUS_FULL: break;
                }
                full_sector_counts++;
                break;
            }
            case SECTOR_STATUS_HEADER_ERROR:
                EFD_DEBUG("Error: Log sector header error! Now will clean all "
                          "log area.\n");
                efd_log_clean();
                return;
        }
    ...
    if (using_sec_counts != 1) {
        ...
        EFD_DEBUG("Error: There must be only one sector status is USING! Now "
                  "will clean all log area.\n");
        efd_log_clean();
    } else {
...
static EfErrCode log_seq_read(uint32_t addr, uint32_t* log, size_t size) {
    ...
    while (size) {
        ...
        result = efd_port_read(addr + read_size, log + read_size / 4,
                               read_size_temp);
...
static uint32_t log_index2addr(size_t index) {
    ...
    size_t sector_num = index / (EFD_ERASE_MIN_SIZE - LOG_SECTOR_HEADER_SIZE)
                        + 1;
...
EfErrCode efd_log_read(size_t index, uint32_t* log, size_t size) {
    ...
    if (index < cur_using_size) {
        EFD_DEBUG("Error: index out of ranges, current using size is %d",
                  cur_using_size);
        return EFD_READ_ERR;
    }
    ...
    if (log_start_addr < log_end_addr) {
        ...
        if (log_index2addr(index) + size
             ...
             */
            read_size_temp = (log_area_start_addr + LOG_AREA_SIZE)
            ...
            if (result == EFD_NO_ERR) {
                result = log_seq_read(log_area_start_addr,
                                      log + read_size_temp / 4,
                                      size - read_size_temp);
            }
        } else {
            /*                          Flash log area
             ...
             */
            result = log_seq_read(log_index2addr(index) - LOG_AREA_SIZE, log,
                                  size);
        }
    }
...
EfErrCode efd_log_write(const uint32_t* log, size_t size) {
    ...
    if ((sector_status == SECTOR_STATUS_USING)
        ...
        writable_size = EFD_ERASE_MIN_SIZE
                        - ((write_addr - log_area_start_addr)
                           % EFD_ERASE_MIN_SIZE);
    ...
    while (true) {
        ...
        if (size - write_size >= writable_size) {
            result = efd_port_write(write_addr, log + write_size / 4,
                                    writable_size);
            if (result != EFD_NO_ERR) {
                goto exit;
            }
