/*
 * app_net_mgm.c (miu-rgb) — RGB 灯带子设备网络管理
 *
 * 基于 miu-mtd 的 MTD 网管流程，额外操作：
 * 在 Challenge/Accepted 握手完成后，立即向 Leader 发送一条 JSON REGISTER 包。
 * 提供 app_rgb_net_report() 供 app_task.c 在执行 CONTROL 后上报当前 RGB 值。
 *
 * JSON 内容：
 *   dev_name = "rgb_01"
 *   dev_type = "RGB"
...
#define RGB_DEV_NAME   "rgb_01"
#define RGB_DEV_TYPE   "RGBCW"
#define RGB_FW_VER     "1.0.0"
#define RGB_HW_VER     "A"
...
/* -----------------------------------------------------------------------
 * 公共发送辅助：获取 inst/ip/rloc16，目标填 Leader RLOC fc00
 * ----------------------------------------------------------------------- */
static otIp6Address mgm_leader_addr(otInstance *inst)
{
...
static int mgm_udp_send_json(const char *json_buf)
{
    otInstance  *inst = otrGetInstance();
    otIp6Address dst  = mgm_leader_addr(inst);
    uint16_t     len  = (uint16_t)strlen(json_buf);
    uint8_t     *buf  = pvPortMalloc(len);
    if (!buf) return -1;
    memcpy(buf, json_buf, len);
    int rc = app_udpSend(dst, buf, len, false);
    vPortFree(buf);
    return rc;
}
 ...
 * JSON REGISTER 发送
 * ----------------------------------------------------------------------- */
static void app_rgb_send_register(void)
{
    otInstance   *inst = otrGetInstance();
    ...
    snprintf(json_buf, sizeof(json_buf),
             "{\"ver\":1,\"type\":\"REGISTER\","
             "\"dev_type\":\"" RGB_DEV_TYPE "\","
             "\"dev_name\":\"" RGB_DEV_NAME "\","
             "\"dev_id\":\"%s\","
             "\"ip\":\"%s\","
             "\"rloc16\":%u,"
             "\"seq\":%u,"
             "\"data\":{\"fw_ver\":\"" RGB_FW_VER "\","
                       "\"hw_ver\":\"" RGB_HW_VER "\"}}",
             dev_id, ip_str, (unsigned)rloc16, (unsigned)s_json_seq);

    if (mgm_udp_send_json(json_buf) != 0) {
        log_info("[rgb] register send fail");
    } else {
        log_info("[rgb] >> REGISTER sent");
        log_info("[rgb]    %s", json_buf);
    }
}

/* -----------------------------------------------------------------------
 * JSON REPORT 发送 — 当前 RGB 值
 * ----------------------------------------------------------------------- */
void app_rgb_net_report(void)
{
    ...
    if (mgm_udp_send_json(json_buf) != 0) {
        log_info("[rgb] report send fail");
    } else {
        log_info("[rgb] >> REPORT R=%u G=%u B=%u", r, g, b);
    }
}
...
void net_mgm_interface_state_change(uint32_t aFlags, void *aContext)
{
    ...
    case OT_DEVICE_ROLE_LEADER:
        log_info("[mgm] became leader (unexpected on RGB device)");
        app_set_led1_on();
        break;
...
{
    log_info("[mgm] RGB device network management init");
