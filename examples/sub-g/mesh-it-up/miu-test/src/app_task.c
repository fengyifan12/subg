/*
 * app_task.c (miu-test) — 测试子设备主任务
 *
 * 基于 miu-mtd，去掉雷达相关初始化。
 * 覆盖 app_udp_comm_json_process() 弱符号：收到 ACK 时打印日志。
 */
...
#define BIN_TYPE_ARR 'm', 'i', 'u', '-', 't', 'e', 's', 't'
 ...
 * 接收来自 Leader 的 ACK 或 CONTROL，打印内容即可
 * ----------------------------------------------------------------------- */
                                ...
                                otIp6Address src_addr)
{
    ...
    if (miu_json_get_str(json, "type",     msg_type, sizeof(msg_type)) < 0 ||
        ...
        log_info("[test] rx: malformed JSON");
        return;
    }
    ...
    if (strcmp(msg_type, "ACK") == 0) {
        ...
        log_info("[test] << ACK  dev=%s  seq_ack=%d  code=%d",
                 dev_name, seq_ack, code);
        if (code == 0) {
            log_info("[test] REGISTER confirmed by leader!");
        }
    } else if (strcmp(msg_type, "CONTROL") == 0) {
        log_info("[test] << CONTROL for %s (ignored in test mode): %s",
                 dev_name, json);
    } else {
        log_info("[test] << unknown type '%s'", msg_type);
    }
}
