/*
 * app_net_mgm.c (miu-socket) — 插座子设备网络管理
 *
...
#define TEST_DEV_NAME   "socket_01"
 ...
 * JSON REGISTER 发送
 * ----------------------------------------------------------------------- */
static void app_socket_send_register(void)
{
    ...
    if (app_udpSend(dst, buf, len, false) != 0) {
        log_info("[socket] register send fail");
    } else {
        log_info("[socket] >> REGISTER sent");
        log_info("[socket]    %s", json_buf);
    }
    vPortFree(buf);
}

/* -----------------------------------------------------------------------
 * JSON REPORT 发送 — 继电器状态
 * ----------------------------------------------------------------------- */
void app_socket_net_report_state(uint8_t state)
{
    otInstance   *inst = otrGetInstance();
    char          ip_str[OT_IP6_ADDRESS_STRING_SIZE];
    char          json_buf[192];
    ...
    snprintf(json_buf, sizeof(json_buf),
             ...
             "\"seq\":%u,"
             "\"data\":{\"state\":%u}}",
             ip_str, (unsigned)rloc16, (unsigned)s_json_seq, (unsigned)state);
    ...
    uint8_t *buf = pvPortMalloc(len);
    if (!buf) {
        log_info("[socket] report: alloc fail");
        return;
    }
    memcpy(buf, json_buf, len);

    if (app_udpSend(dst, buf, len, false) != 0) {
        log_info("[socket] report state=%u send fail", (unsigned)state);
    } else {
        log_info("[socket] >> REPORT state=%u", (unsigned)state);
    }
    vPortFree(buf);
}
...
{
    log_info("[mgm] socket device network management init");
