...
#if EFD_WRITE_GRAN != 1 && EFD_WRITE_GRAN != 8 && EFD_WRITE_GRAN != 32         \
    && EFD_WRITE_GRAN != 64
...
#define STATUS_TABLE_SIZE(status_number)                                       \
    ((status_number * EFD_WRITE_GRAN + 7) / 8)
#else
#define STATUS_TABLE_SIZE(status_number)                                       \
    (((status_number - 1) * EFD_WRITE_GRAN + 7) / 8)
#endif
...
#if (EFD_GC_EMPTY_SEC_THRESHOLD == 0                                           \
     || EFD_GC_EMPTY_SEC_THRESHOLD >= SECTOR_NUM)
...
struct sector_hdr_data {
    struct {
        uint8_t store
            [STORE_STATUS_TABLE_SIZE]; /**< sector store status @see sector_store_status_t */
        uint8_t dirty
            [DIRTY_STATUS_TABLE_SIZE]; /**< sector dirty status @see sector_dirty_status_t */
    } status_table;

    uint32_t magic; /**< magic word(`E`, `F`, `4`, `0`) */
    uint32_t
        combined; /**< the combined next sector number, 0xFFFFFFFF: not combined */
    uint32_t reserved;
...
struct env_hdr_data {
    uint8_t status_table
        [ENV_STATUS_TABLE_SIZE]; /**< ENV node status, @see node_status_t */
    uint32_t magic;              /**< magic word(`K`, `V`, `4`, `0`) */
    uint32_t
        len; /**< ENV node total length (header + name + value), must align by EFD_WRITE_GRAN */
...
#if (EFD_WRITE_GRAN == 1)
    result = efd_port_write(addr + byte_index,
        ...
        if ((empty_addr > sec_addr) && (empty_addr < sec_addr + SECTOR_SIZE)) {
            ...
            if (sector_cache_table[i].addr == sec_addr) {
                ...
                return;
                       ...
                       && (empty_index == EFD_SECTOR_CACHE_TABLE_SIZE)) {
                empty_index = i;
            }
...
static void update_env_cache(const char* name, size_t name_len, uint32_t addr) {
    size_t i, empty_index = EFD_ENV_CACHE_TABLE_SIZE,
              min_activity_index = EFD_ENV_CACHE_TABLE_SIZE;
        ...
        if (addr != FAILED_ADDR) {
            ...
            if (env_cache_table[i].name_crc == name_crc) {
                ...
                return;
                       ...
                       && (empty_index == EFD_ENV_CACHE_TABLE_SIZE)) {
                empty_index = i;
            } else if (env_cache_table[i].addr != FAILED_ADDR) {
...
static bool get_env_from_cache(const char* name, size_t name_len,
    ...
    for (i = 0; i < EFD_ENV_CACHE_TABLE_SIZE; i++) {
        if ((env_cache_table[i].addr != FAILED_ADDR)
            ...
            efd_port_read(env_cache_table[i].addr + ENV_HDR_DATA_SIZE,
                          (uint32_t*)saved_name, EFD_ENV_NAME_MAX);
            if (!memcmp(name, saved_name, name_len)) {
                *addr = env_cache_table[i].addr;
                if (env_cache_table[i].active
                    >= 0xFFFF - EFD_ENV_CACHE_TABLE_SIZE) {
...
static uint32_t find_next_env_addr(uint32_t start, uint32_t end) {
    ...
    if (get_sector_from_cache(EFD_ALIGN_DOWN(start, SECTOR_SIZE), &empty_env)
        && start == empty_env) {
        return FAILED_ADDR;
    }
#endif /* EFD_ENV_USING_CACHE */
...
static EfErrCode read_env(env_node_obj_t env) {
    ...
    efd_port_read(env->addr.start, (uint32_t*)&env_hdr,
                  sizeof(struct env_hdr_data));
    env->status = (env_status_t)get_status(env_hdr.status_table,
                                           ENV_STATUS_NUM);
    env->len = env_hdr.len;
    ...
    for (len = 0, size = 0; len < crc_data_len; len += size) {
        ...
        efd_port_read(env->addr.start + ENV_NAME_LEN_OFFSET + len,
                      (uint32_t*)buf, EFD_WG_ALIGN(size));
    ...
    if (calc_crc32 != env_hdr.crc32) {
        ...
        efd_port_read(env_name_addr, (uint32_t*)env->name,
                      EFD_WG_ALIGN(env_hdr.name_len));
    ...
    if (traversal) {
        ...
        if (sector->status.store == SECTOR_STORE_EMPTY) {
            ...
            if (get_sector_from_cache(addr, &sector->empty_env)) {
                sector->remain = SECTOR_SIZE
                                 - (sector->empty_env - sector->addr);
                return result;
            }
#endif /* EFD_ENV_USING_CACHE */
 ...
 */
size_t efd_get_env_blob(const char* key, void* value_buf, size_t buf_len,
...
char* efd_get_env(const char* key) {
    ...
    if ((get_size = efd_get_env_blob(key, value, EFD_STR_ENV_VALUE_MAX_SIZE,
                                     NULL))
        > 0) {
 ...
 */
size_t efd_read_env_value(env_node_obj_t env, uint8_t* value_buf,
                          size_t buf_len) {
    size_t read_len = 0;
...
static EfErrCode write_env_hdr(uint32_t addr, env_hdr_data_t env_hdr) {
    ...
    result = efd_port_write(addr + ENV_MAGIC_OFFSET, &env_hdr->magic,
...
static EfErrCode format_sector(uint32_t addr, uint32_t combined_value) {
    ...
    if (result == EFD_NO_ERR) {
        ...
        /* save the header */
        result = efd_port_write(addr, (uint32_t*)&sec_hdr,
...
static EfErrCode update_sec_status(sector_meta_data_t sector,
    ...
    if (sector->status.store == SECTOR_STORE_EMPTY) {
        ...
        /* check remain size */
        if (sector->remain < EFD_SEC_REMAIN_THRESHOLD
            || sector->remain - new_env_len < EFD_SEC_REMAIN_THRESHOLD) {
...
static EfErrCode del_env(const char* key, env_node_obj_t old_env,
    ...
    dirty_status_addr = EFD_ALIGN_DOWN(old_env->addr.start, SECTOR_SIZE)
                        + SECTOR_DIRTY_OFFSET;
...
static EfErrCode move_env(env_node_obj_t env) {
    ...
    /* start move the ENV */
    {
        ...
        for (len = 0, size = 0; len < env_len; len += size) {
            ...
            efd_port_read(env->addr.start + ENV_MAGIC_OFFSET + len,
                          (uint32_t*)buf, EFD_WG_ALIGN(size));

            result = efd_port_write(env_addr + ENV_MAGIC_OFFSET + len,
                                    (uint32_t*)buf, size);
        }
        ...
        update_sector_cache(EFD_ALIGN_DOWN(env_addr, SECTOR_SIZE),
                            env_addr + ENV_HDR_DATA_SIZE
                                + EFD_WG_ALIGN(env->name_len)
                                + EFD_WG_ALIGN(env->value_len));
...
static uint32_t new_env_by_kv(sector_meta_data_t sector, size_t key_len,
                              size_t buf_len) {
    size_t env_len = ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(key_len)
                     + EFD_WG_ALIGN(buf_len);
...
static bool do_gc(sector_meta_data_t sector, void* arg1, void* arg2) {
    ...
    if (sector->check_ok
        && (sector->status.dirty == SECTOR_DIRTY_TRUE
...
static EfErrCode align_write(uint32_t addr, const uint32_t* buf, size_t size) {
    ...
    align_remain = EFD_WG_ALIGN_DOWN(
    ...
    if (result == EFD_NO_ERR && align_remain) {
        memcpy(align_data, (uint8_t*)buf + EFD_WG_ALIGN_DOWN(size),
               align_remain);
        result = efd_port_write(addr + EFD_WG_ALIGN_DOWN(size),
...
static EfErrCode create_env_blob(sector_meta_data_t sector, const char* key,
    ...
    env_hdr.len = ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(env_hdr.name_len)
                  + EFD_WG_ALIGN(env_hdr.value_len);
    ...
    if (env_addr != FAILED_ADDR
        ...
        if (result == EFD_NO_ERR) {
            result = align_write(
                env_addr + ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(env_hdr.name_len),
 ...
 * @return result
 */
EfErrCode efd_del_and_save_env(const char* key) { return efd_del_env(key); }
 ...
 * @return result
 */
EfErrCode efd_set_env_blob(const char* key, const void* value_buf,
                           size_t buf_len) {
    EfErrCode result = EFD_NO_ERR;
        ...
        if (env->status == ENV_WRITE) {
            ...
            __reload:
                ...
                for (len = 0, size = 0; len < env->value_len; len += size) {
                    ...
                    efd_port_read(env->addr.value + len, (uint32_t*)buf,
                                  EFD_WG_ALIGN(size));
                    if (print_value) {
                        log_info("%.*s", size, buf);
            ...
            if (value_is_str && !print_value) {
                ...
                log_info("blob @0x%08X %dbytes", env->addr.value,
                         env->value_len);
            }
            log_info("\n");
        }
...
void info_env(void) {
    ...
    log_info(
        "size: %lu/%lu bytes.\n",
        using_size
            + (SECTOR_NUM - EFD_GC_EMPTY_SEC_THRESHOLD) * SECTOR_HDR_DATA_SIZE,
